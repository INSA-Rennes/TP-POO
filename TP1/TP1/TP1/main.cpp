...
int main() {
    ...
    for(int icoul=3; icoul>=0; icoul--) {
        for(int ihaut=12; ihaut>=0; ihaut--) {
            ...
            Hauteur hauteur = static_cast<Hauteur>(ihaut + 1);
            ...
            new Carte(couleur, hauteur, proprietaire);
        }
    }
    ...
    while(Carte::getNTete()!=0 && Carte::getSTete()!=0) {
		...
		std::cout << "Carte de N : " << std::endl;
        Nt->afficher();
		std::cout << std::endl;
		std::cout << "Carte de S : " << std::endl;
        St->afficher();
		std::cout << std::endl;
        std::cout << std::endl;
